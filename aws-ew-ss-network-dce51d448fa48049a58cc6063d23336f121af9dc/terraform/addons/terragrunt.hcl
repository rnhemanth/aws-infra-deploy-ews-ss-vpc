locals {
  ...
  remote_state_bucket = "${local.env}-${local.service}-tf-state-${local.aws_account_id}"
  state_lock_table    = "${local.env}-${local.service}-tf-locks-${local.aws_account_id}"
  remote_state_key    = "core-infra-addons/${local.env}-${local.service}.tfstate"
